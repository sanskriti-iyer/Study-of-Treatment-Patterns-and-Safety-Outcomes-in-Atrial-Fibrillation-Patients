/* Summary of change in CV -- adapted from analysis/CV_Change.sas.
   Macro %stat_calculate and the PROC REPORT are the author's, unchanged.
   The only edits: read the sample table code_vital_sign_analysis (defined in
   autoexec.sas) in place of the external code.vital_sign_analysis, and write
   the ODS output to the working directory instead of an absolute Reports path. */
%macro stat_calculate (loinc =, label =, ord =);
    data vs3;
        set code_vital_sign_analysis;
        where loinc = "&loinc.";
    run;
...
ods pdf file = 'CV_Change.pdf' style = Journal;
