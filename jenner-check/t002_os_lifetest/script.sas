/* Overall survival summary -- adapted from analysis/OS_Summary.sas.
   The PROC LIFETEST (Kaplan-Meier quartiles), the frequency/censoring
   tabulations, and the PROC REPORT are the author's, unchanged. The only
   edits: read the sample table code_os (defined in autoexec.sas) in place of
   the external code.os, and write ODS output to the working directory. */
proc lifetest data=code_os timelim=100 outsurv=os_surv reduceout plots=none;
    strata cohort;
    time AVAL * CNSR(1);
    ods output Quartiles=qrts;
run;

proc freq data = code_os noprint;
...
proc sql noprint;
    select count(*) into :T_Tot trimmed from code_os;
quit;
...
ods pdf file = 'OS_Summary.pdf' style = Sapphire;
